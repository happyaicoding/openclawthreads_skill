---
name: social-post
description: 學使用者的寫作風格（從本機 MD 檔），依 14 天內容策略日曆，自動產出並發佈到 Instagram + Threads（API-only）。使用時機：使用者說「發文」、「幫我寫一篇貼文」、「用我的風格發」、「今天發一篇」、「PO 一下」、「學我的語氣」、「分析我的貼文風格」、「重新規劃內容」、「排貼文」、「查流量」、「review」、「讀 MD 資料夾」時一律觸發。
version: 1.0.0-api
emoji: 📱
homepage: https://github.com/Hao0321/claude-skill-social-post
requires:
  env:
    - CONFIG_FILE_PATH
---
...
| 觸發 | 階段 | 讀 | 備註 |
|---|---|---|---|
| `style_profile.md` 不存在 / 說「重新學風格」/ 說「讀 MD 資料夾」 | **P1** | `references/learn_style.md` | 僅支援 MD 資料夾（Option B），已移除 FB 爬取 |
| `content_plan.md` 不存在 / 說「重新規劃」「排新的 14 天」 | **P0** | `references/phase0_plan.md` + `formulas.md` | 同前版本 |
| 說「發文」「今天發一篇」「PO」 | **P2** | `references/generate_and_publish.md` + `style_profile.md` + `content_plan.md` + `formulas.md`（目標公式段）+ 目標平台 ref（IG 或 Threads） | **API-only**：讀 `instagram.md` 或 `threads.md` 學 API 流程，不用 browser-automation |
| 說「這篇好不好」「查流量」「分析」 | **診斷** | `references/evaluation.md`（評估框架）+ 目標 post 戰績 | 同前版本 |
| 說「歷史怎麼樣」「Day X 發生什麼」 | **案例** | `references/case_studies.md`（Day 1-4 歸納）| 同前版本 |

路由前用一句話告知使用者要做哪階段，給糾正機會。

### CONFIG_FILE_PATH 環境變數

發文時（P2 階段）必須設置 `CONFIG_FILE_PATH` 環境變數指向 `config.json` 的完整路徑。

```bash
export CONFIG_FILE_PATH="/path/to/config.json"
```

Skill 會自動讀取此路徑，取得 Instagram + Threads API 認證信息。若路徑無效或 `config.json` 不存在，發文時停止並告知使用者。

## 先決條件（API-only 版本）

- `config.json` 存在且包含有效的 Instagram + Threads API 認證信息（見 `config.example.json`）
- Token 自動更新機制已啟用（`token_refresh.auto_refresh: true`）
- 使用者已升級 Instagram 帳號到 Graph API（見 `references/ig-graph-api-upgrade.md`）
- **無須瀏覽器自動化**（純 API 呼叫）
...
- 跨平台同一段複製（每平台重新生成）
- 自動按讚 / 回覆 / follow / 大量留言
- 外傳 `config.json` / `style_profile.md` / 使用者資料（**特別重要**：不要在日誌中暴露 API token）
- 在程式碼或聊天中暴露 API token（若意外暴露，立即告知使用者重新核發）
- 硬編碼 token 或密鑰（僅從 `config.json` 讀取）
- 刪除使用者留言 / 貼文（系統硬規則）
- 發文到除了 Instagram + Threads 外的平台（本版本不支援 FB / X）
...
| 需要 | 去 |
|---|---|
| **雙平台發文腳本（IG + Threads）** | **`post_to_social.py`** |
| Instagram Graph API 完整指南 | `references/instagram.md` |
| Threads 發文參數與 API 步驟 | `references/threads.md` |
| Token 刷新、故障排除 | `references/api-integration.md` |
| Instagram Graph API + Threads Meta API 發文流程 | `references/api-flow.md` |
| 風格學習（MD 資料夾讀取） | `references/learn_style.md` |
| 2026 演算法訊號權重 + 4 指標評估框架 | `references/evaluation.md` |
| Day 1-4 實戰案例解剖 | `references/case_studies.md` |
| 19 個公式（F1-F19）與 Thread 限制 | `references/formulas.md` |
...
- **Token 過期**：若收到 401 Unauthorized，檢查 `config.json` 的 `expires_at`。若已過期，自動執行 token refresh（見 `api-integration.md`）。若 refresh 失敗，停止發文並告知使用者需重新授權。
- **Token 暴露**：若使用者不小心貼出真實 token 到聊天，**立即停止**，告知使用者這個 token 已暴露，必須在 Meta App Dashboard 重新核發。
- **Graph API 未升級**：IG 帳號若還在「Basic Display API」，無法發文。檢查 `references/ig-graph-api-upgrade.md` 協助升級。

### Instagram API

- **圖片必須**：IG 純文字發不了，一律提醒改 Threads（支援純文字）
- **圖片格式**：支援 JPG / PNG / GIF，不支援 HEIC。若上傳失敗，檢查檔案格式與大小
- **速率限制**：發完一篇等至少 30 秒再發下一篇，否則觸發 429 Too Many Requests
- **帳號升級缺漏**：若帳號無法發文，確認已升級至 Graph API（不是 Basic Display）

### Threads API

- **字數限制**：> 500 字必須分串。分串前逐則預覽確認，避免發到一半失敗而整串遺失。
- **分串邏輯**：第一則是鉤子 + 主觀點，後續補細節，**必須在句子邊界切**（不要無謂斷句）
- **速率限制**：兩篇間至少 10 秒，分串時每則間 2-3 秒
- **多帳號**：Threads API token 綁定特定帳號，多帳號時需切換 token（更新 `config.json`）

### 一般發文

- **IG 要圖，Threads 不用**：若無圖片，給使用者選擇 (A) 提供圖片 (B) 跳過 IG (C) 改 Threads
- **每平台重新生成**：不要跨平台複製同一段內容，各平台各生一版（IG 視覺化、Threads 口語短句）
- **API 故障**：若 API 返回 5xx，通常是暫時故障。等待後重試或告知使用者稍候
- **使用者 override 規則**（實戰遇過）：
  - 使用者明確 override（例：「一樣 2:13 發」即使 R5 禁 F6b 7 天內）→ **執行 + 標記 override 在戰績備註** + 降風險策略（如混搭新意圖）
  - 不重複爭論 risk（已說過一次即可）
