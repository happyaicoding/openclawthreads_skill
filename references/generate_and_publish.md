...
6. **依序發佈**：讀目標平台的 `references/{instagram,threads}.md`，照 API 步驟發。
   - **所有發文皆使用 API（配置讀 config.json）**
   - Instagram 使用 Graph API（長期 Access Token + 自動更新機制）
   - Threads 使用 Meta API（同 Instagram 認證）
7. **每發完一個回報**：「✓ 已發到 ＜平台＞；連結：＜API 返回的 post URL＞」。
8. **任何一平台失敗就停手**：告訴使用者（已發哪些、哪個卡住、看到什麼錯誤），等指示。**不要跳過繼續下一個**。檢查 `references/api-integration.md` 的故障排除章節。
9. **發完追蹤**：問使用者「要不要等幾小時後幫你看一下數據？」——若要，把「今天發完後 N 小時回看」記 note。
10. **更新 `content_plan.md`**：
    - 把「最近發文日期」改成今天
    - 「今天在哪一天」+1（若到 14 就回 1）
    - 戰績表追加一行（讚/留/分享先空著，事後再填）
    - **發文連結欄位記錄 API 返回的 post URL**
